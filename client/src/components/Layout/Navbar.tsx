...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-50 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} shadow-lg md:shadow-none md:top-0 md:bottom-auto`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between py-2 md:py-4 max-w-4xl mx-auto">
          {navItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
