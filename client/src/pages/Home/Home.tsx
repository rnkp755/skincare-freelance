...
const Home: React.FC = () => {
  const { user } = useAuth();
  const { completionRate, pendingSteps, todaysRoutine, canLogRoutine, logRoutineStep } = useLog();
  const navigate = useNavigate();
  
  const handleRoutineComplete = (stepId: string, completed: boolean, photoUrl?: string, reason?: string) => {
  ...
  const currentStep = pendingSteps.find(step => canLogRoutine(step.timeOfDay));
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {currentStep && (
        <Card className="bg-gradient-to-r from-teal-500 to-blue-500 text-white animate-fade-in">
          <h2 className="text-xl font-semibold mb-4">Complete Today's Routine</h2>
          <RoutineForm 
            step={currentStep}
            onComplete={(completed, photoUrl, reason) => 
              handleRoutineComplete(currentStep.id, completed, photoUrl, reason)
            }
          />
        </Card>
      )}
      ...
      <Card className="animate-fade-in">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="mb-4">
              You've completed <span className="font-semibold text-teal-600 dark:text-teal-400">{Math.round(completionRate)}%</span> of your skincare routine steps.
              {completionRate > 75 ? 
      ...
      <Card className="animate-fade-in delay-75">
        <h2 className="text-xl font-semibold mb-4">Recent Consultation</h2>
        <div 
        ...
        >
          ...
          <p className="mt-2 text-sm line-clamp-2">
            Based on your photos, I've identified mild acne with some dryness. I've suggested a gentle routine with moisturizing and acne-fighting ingredients...
          </p>
        </div>
      </Card>
    </div>
