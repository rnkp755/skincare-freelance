...
const Auth: React.FC = () => {
  ...
  const handleSkip = () => {
    ...
    if (mode === 'login') {
      ...
      signup('Demo User', 'demo@example.com', 'password').then(() => navigate('/'));
    }
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <div className="absolute top-4 right-4">
        <button
          onClick={toggleTheme}
          className={`p-2 rounded-full ${theme === 'dark' ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-700'} shadow-md`}
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        <div className={`w-full max-w-md p-8 rounded-xl shadow-xl ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
