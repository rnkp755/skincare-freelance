...
interface LogContextType {
  ...
  logRoutineStep: (stepId: string, completed: boolean, photoUrl?: string, reason?: string) => void;
...
const mockRoutines: Routine[] = [
  {
    ...
    steps: [
      { id: 's1', description: 'Wash face with gentle cleanser', timeOfDay: 'morning' },
      { id: 's2', description: 'Apply vitamin C serum', timeOfDay: 'morning' },
      { id: 's3', description: 'Apply moisturizer with SPF', timeOfDay: 'morning' },
      { id: 's4', description: 'Wash face with gentle cleanser', timeOfDay: 'evening' },
      { id: 's5', description: 'Apply retinol serum', timeOfDay: 'evening' },
      { id: 's6', description: 'Apply night moisturizer', timeOfDay: 'evening' },
    ],
    medicines: [
      { 
        id: 'm1', 
  ...
  },
  {
    ...
    steps: [
      { id: 's7', description: 'Wash face with gentle cleanser', timeOfDay: 'morning' },
      { id: 's8', description: 'Apply vitamin C serum', timeOfDay: 'morning' },
      { id: 's9', description: 'Apply moisturizer with SPF', timeOfDay: 'morning' },
      { id: 's10', description: 'Wash face with gentle cleanser', timeOfDay: 'evening' },
      { id: 's11', description: 'Apply retinol serum', timeOfDay: 'evening' },
      { id: 's12', description: 'Apply night moisturizer', timeOfDay: 'evening' },
    ],
    medicines: [
      { 
        id: 'm4', 
...
const mockLogs: LogEntry[] = [
  {
    ...
    stepId: 's7',
    completed: true,
    photoUrl: 'https://images.pexels.com/photos/3762453/pexels-photo-3762453.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
...
export const LogProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Get today's routine
  const todaysRoutine = routines.find(r => r.date === format(new Date(), 'yyyy-MM-dd')) || null;
  ...
  const canLogRoutine = (timeOfDay: 'morning' | 'afternoon' | 'evening'): boolean => {
    const now = new Date();
  ...
  // Log a routine step
  const logRoutineStep = (stepId: string, completed: boolean, photoUrl?: string, reason?: string) => {
